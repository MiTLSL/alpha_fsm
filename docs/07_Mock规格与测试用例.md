...
L3-SIM  仿真端到端测试（geometry profile + light physics profile）
    - 真 TF + 真值视觉/点云 + 完美底盘执行
    - FINE_ALIGN 收敛、MoveIt/IK dry_run、多墙几何闭环
    - L3-SIM-03 使用 Gazebo 轻量物理场景验证机器人 / 场地 / 纸箱 / 控制链路
...
每层用例必须**独立可跑**，不依赖上一层。L0/L1 在 CI 跑；L2/L3 本地或在带显示的 runner 跑；L3-SIM 依赖 MoveIt/ros2_control 时放 nightly 或仿真 runner 跑。
...
M2-SIM 使用测试专用节点和仿真启动项模拟外部世界，不改变生产节点接口。它和 mock 的区别是：mock 直接返回脚本化结果，sim 通过 TF、FOV、运动学积分、MoveIt / ros2_control 和 Gazebo 轻量物理场景产生可复盘的闭环。

L3-SIM-01/02 仍以几何/运动学 profile 为主，优先保持快速、稳定、低依赖；L3-SIM-03 切到轻量物理 profile，引入 Gazebo 场景、箱体碰撞/重力/摩擦和 ros2_control 仿真控制链。抓取接触判定与固定约束属于阶段 B 增量，不作为 L3-SIM-03 第一版阻塞验收。

| sim 节点 / 启动项 | 替代或模拟对象 | 主要用途 | 是否生产依赖 |
|---|---|---|---|
| `sim_world_node` | 5×5 箱墙、相机 FOV、雷达点云真值 | 维护箱体 6D pose、可见性、RViz marker，发布真值感知输入 | 否 |
| `fake_nav2_base_node` | Nav2 coarse goal + 底盘运动学 | goal 完美到位；FINE_ALIGN 阶段消费 `/cmd_vel_align` 并更新 odom/TF | 否 |
| `moveit_mock_hardware` launch | ros2_control mock_components | 几何/运动学 profile 中让真实 `pair_grasp_execution_node` 能 dry_run 验证 IK/轨迹规划 | 否 |
| Gazebo light physics launch | 场地、集装箱、纸箱、机器人仿真控制链 | L3-SIM-03 第一版验证 Gazebo world、箱体基础物理、MoveIt、ros2_control 和错误映射 | 否 |
| `grasp_constraint_bridge`（阶段 B） | TCP / box 接触条件与固定约束 | 抓取成功/失败规则、释放规则和约束失败注入；不直接进入生产 FSM | 否 |
| `bringup_with_kinematic_sim.launch.py` | 几何/运动学 profile | 组合 sim world、fake base、真实/模拟 adapter、MoveIt mock hardware | 否 |
| `bringup_with_light_physics_sim.launch.py`（建议新增） | 轻量物理 profile | 组合 Gazebo world、ros2_control、MoveIt、mock 真值感知、FSM 核心节点和 Web/Foxglove 观测 | 否 |

sim 节点可以复用 `InjectFailure.srv` 做测试注入，但不得新增生产错误码，也不得让生产节点订阅 `/sim/*` 私有调试 Topic。
...
| 用例 ID | 场景 | 步骤 / 预期 |
|---|---|---|
| L3-SIM-01 | 真 TF + 真值感知单墙 happy | `sim_world_node` 发布 5×5 箱墙；优先走 `/box_perception/result` → `perception_adapter_node` → `/perception/box_detections`；若当前环境缺上游 msg，则用旁路模式直接发 `/perception/box_detections`；WallMappingFSM 拟合的 wall_frame 稳定，grid.slots=25 |
| L3-SIM-02 | FINE_ALIGN 收敛性 | `fake_nav2_base_node` 让 LEFT_PHASE/RIGHT_PHASE 初始位带 ±10cm / ±5° 扰动；真实 `navigation_manager_node` 通过 `/cmd_vel_align` 闭环；`alignment_error_current` 在 timeout 内收敛到 `business.fine_alignment.*` 阈值 |
| L3-SIM-03 | 轻量物理抓取链路第一版 | Gazebo world 加载机器人、地面、集装箱和至少 1 个纸箱；纸箱有基础重力、碰撞、摩擦；真实 `pair_grasp_execution_node` 接 MoveIt 2 + ros2_control 仿真链路；mock 真值感知输出目标；标准 pair 完成规划、接近、预抓 dry_run；工作空间边缘 pair 触发 5200/5201/5210 中对应错误并映射 recovery |
| L3-SIM-04 | 半真组合端到端 | sim perception + real navigation adapter + real grasp dry_run + mock vacuum 完整跑单墙，任务 success=true；失败时 rosbag 中能定位到 TF、FINE_ALIGN 或 IK 阶段 |
| L3-SIM-05 | 多墙 / 长跑回归 | 连续 10 面墙或 time_scale=10 跑等价长流程；检查内存、日志、rosbag 体量增长趋势，无 OOM，无 log 爆量 |

L3-SIM 的目的不是验证传感器算法质量。视觉、雷达、吸盘流体、柔性箱体形变仍属于 L4 现场联调或专项测试。L3-SIM-03 阶段 B 可以增加“接触条件 + 固定约束”的仿真抓取桥接器，但它只验证简化抓取规则和业务错误映射，不证明真实吸附可靠性。

### 8.6 错误码覆盖矩阵

下列 P0 错误码必须至少有一条 L2、L3 或 L3-SIM 用例命中并验证 recovery：
...
| 4040 (fine align fail) | L2-NAV-04 + L3-SIM-02 |
| 4042 (fine align no feedback) | L3-SIM-02（断流变体） |
...
| 5200 (IK fail) | L3-E2E-02 + L3-SIM-03 |
| 5201 (trajectory fail) | L3-SIM-03 |
| 5210 (collision detected) | L3-SIM-03 |
...
│   │   └── nav_results.py
│   ├── sim/
│   │   ├── sim_world_node.py             # M2-SIM：箱墙真值、FOV、marker、真值感知输入
│   │   ├── fake_nav2_base_node.py        # M2-SIM：粗导航 + /cmd_vel_align 运动学积分
│   │   ├── scene_loader.py               # 场景 YAML 读取与多墙生成
│   │   └── grasp_constraint_bridge.py    # 阶段 B：仿真接触判定与固定约束桥接
│   └── helpers/
...
│   ├── bringup_with_kinematic_sim.launch.py
│   ├── bringup_with_light_physics_sim.launch.py  # 建议新增：L3-SIM-03 轻量物理 profile
...
| 触发 | L0 | L1 | L2 | L3 | L3-SIM |
|---|---|---|---|---|---|
| 每 PR | ✅ | ✅ | ✅（核心 5 条）| ❌ | ❌ |
| 主分支 push | ✅ | ✅ | ✅（全部）| ✅（核心 3 条 happy）| 可选，仅在仿真依赖齐全 runner 跑 L3-SIM-01 |
| 每日定时 | ✅ | ✅ | ✅ | ✅（全部）| ✅（L3-SIM-01/02/03）|
...
- L3-SIM：额外录 `/tf`、`/tf_static`、`/cmd_vel_align`、`/odom`、MoveIt planning scene / joint states，以及 `/sim/*` 私有调试 Topic；L3-SIM-03 还需保留 Gazebo world 日志、model states / contact 调试信息和 Foxglove/RViz 观测布局
...
- L3-SIM-02：FINE_ALIGN 在 15s timeout 内收敛，收敛后连续 `pass_frames` 帧满足阈值
- L3-SIM-03：标准 pair 的 IK + 轨迹规划单次耗时 ≤ 2s（仿真 runner 上统计 P95）；Gazebo 轻量物理场景启动成功率进入 nightly 稳定性统计
...
- v1.1 2026-05-27：新增 L3-SIM 运动学仿真分层、节点清单、用例集、CI/产物规则
- v1.2 2026-06-11：L3-SIM-03 升级为轻量物理仿真第一版；接触固定约束放入阶段 B，不作为第一版阻塞验收
- 后续每加一个 P0 错误码，必须同步更新 §8.6 矩阵
