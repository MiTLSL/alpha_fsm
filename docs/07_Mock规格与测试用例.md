...
L3-SIM  运动学仿真端到端测试（bringup_with_kinematic_sim）
    - 真 TF + 真值视觉/点云 + 完美底盘执行
...
M2-SIM 使用测试专用节点模拟外部世界，不改变生产节点接口。它和 mock 的区别是：mock 直接返回脚本化结果，sim 通过 TF、FOV、运动学积分和 MoveIt mock hardware 产生可复盘的几何闭环。
...
| `moveit_mock_hardware` launch | ros2_control mock_components | 让真实 `pair_grasp_execution_node` 能 dry_run 验证 IK/轨迹规划 | 否 |
| `bringup_with_kinematic_sim.launch.py` | 仿真 profile | 组合 sim world、fake base、真实/模拟 adapter、MoveIt mock hardware | 否 |
...
### 8.5 L3-SIM 运动学仿真用例（bringup_with_kinematic_sim）
...
| L3-SIM-03 | MoveIt/IK dry_run | 真实 `pair_grasp_execution_node` 接 MoveIt 2 + ros2_control mock_components；标准 pair 规划成功；工作空间边缘 pair 触发 5200/5201/5210 中对应错误并映射 recovery |
...
L3-SIM 的目的不是验证传感器算法质量。视觉、雷达、吸附、接触动力学仍属于 L4 现场联调或专项测试。
...
- L3-SIM：额外录 `/tf`、`/tf_static`、`/cmd_vel_align`、`/odom`、MoveIt planning scene / joint states，以及 `/sim/*` 私有调试 Topic；失败时保留 RViz marker bag
...
- L3-SIM-03：标准 pair 的 IK + 轨迹规划单次耗时 ≤ 2s（仿真 runner 上统计 P95）
