...
flowchart LR
    ...
    Mapping --> Phase["选择左 / 右 phase"]
    Phase --> Workpose["导航到 phase 工位<br/>可带 FINE_ALIGN"]
    Workpose --> LocalPerc["局部感知<br/>更新 slot pose"]
    LocalPerc --> SelectPair["选择抓取 pair"]
    SelectPair --> Grasp["执行双臂抓取"]
    Grasp --> Update["更新网格状态"]
    Update --> MorePair{"当前 phase<br/>还有可抓箱?"}
    MorePair -- 是 --> LocalPerc
    MorePair -- 否 --> MorePhase{"还有 phase?"}
    MorePhase -- 是 --> Phase
    MorePhase -- 否 --> Verify["复核任务结果"]
    Verify --> Done["任务完成"]
```
...
| pair_grasp_execution | 已实现 14 阶段主线、GraspPair 校验、dry_run / fake_real / real MoveGroup Action 骨架、cancel / estop | `scripts/m2_pre_grasp_fake_real_smoke.py` | MoveIt mock hardware / 真机 dry-run / 真空硬件 |
