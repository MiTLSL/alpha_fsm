...
- L3-ISAAC 中本节点仍只认 MoveIt / controller / `/joint_states` / vacuum 契约；Isaac 机械臂 articulation 和仿真抓取约束后续通过 `isaac_joint_trajectory_bridge_node`、`isaac_grasp_bridge_node` 扮演下游控制器和真空后端。
...
Isaac Sim 全流程仿真不改变 `/execute_pair_grasp` Action，也不让策略层直接控制 Isaac articulation。推荐链路：

```text
strategy_node
  → /execute_pair_grasp
  → pair_grasp_execution_node
  → MoveIt / FollowJointTrajectory / vacuum
  → isaac_joint_trajectory_bridge_node / isaac_grasp_bridge_node
  → Isaac robot articulation / box constraint
```

当前仓库已完成 L3-ISAAC 前三步中的 groundTruth 感知和底盘状态桥。机械臂轨迹桥和抓取约束桥要等 Isaac Sim 安装、机器人 USD articulation 与 URDF/SRDF 关节名对齐后再实现。
