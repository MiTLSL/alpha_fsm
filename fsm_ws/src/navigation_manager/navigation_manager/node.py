...
def _make_pose_stamped(frame_id: str = "map", x: float = 0.0, y: float = 0.0, z: float = 0.0):
...
class NavigationManagerNodeMixin:
    ...
    async def execute_navigation_goal(self, goal_handle):
        ...
        result.actual_base_pose = _make_pose_stamped(target_pose.header.frame_id)
    ...
    def _on_nav2_feedback(self, goal_handle, feedback_msg) -> None:
        ...
        self._last_nav2_eta_sec = _duration_to_sec(getattr(feedback, "estimated_time_remaining", None))
    ...
    def _pose_error(self, target_pose, actual_pose) -> tuple[float, float]:
        ...
        yaw_error = abs(_angle_delta(_yaw_from_pose(target_pose), _yaw_from_pose(actual_pose)))
